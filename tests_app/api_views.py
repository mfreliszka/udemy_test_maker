...
def api_create_question(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # AUTO-SELECT DOMAIN using DomainSuggester
            domain = None
            domain_suggestion = None
            question_text = data['question_text']
            
            try:
                from .utils.domain_suggester import get_auto_domain_for_question
                
                # Get best domain suggestion with minimum 40% confidence
                best_domain = get_auto_domain_for_question(question_text, exam.id)
                
                # If no confident domain found, get the best one anyway (even with low confidence)
                # if not best_domain:
                #     from .utils.domain_suggester import DomainSuggester
                #     suggester = DomainSuggester()
                #     suggestions = suggester.get_domain_suggestions_for_exam(question_text, exam.id, top_n=1)
                #     if suggestions and len(suggestions) > 0:
                #         best_suggestion = suggestions[0]
                #         if best_suggestion.get('domain_id'):
                #             best_domain = best_suggestion

                if best_domain:
                    domain_suggestion = {
                        'domain_name': best_domain['domain_name'],
                        'confidence': best_domain['confidence'],
                        'confidence_level': best_domain['confidence_level'],
                        'score': best_domain.get('score', 0)
                    }
                    
                    # Get the actual domain object
                    if best_domain.get('domain_id'):
                        try:
                            domain = Domain.objects.get(id=best_domain['domain_id'], is_active=True)
                        except Domain.DoesNotExist:
                            domain = None
                            
            except Exception as e:
                print(f"Domain suggestion error: {e}")  # Log error but continue without domain
                domain_suggestion = None
