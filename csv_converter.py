...
def determine_correct_answers(question_element, question_type):
    ...
    if question_type == "multiple-choice":
        # For multi-select, extract from "Unattempted" section text specifically
        explanation_div = question_element.find('div', class_='wpProQuiz_response')
        if explanation_div:
            # Look specifically in the unattempted div
            unattempted_div = explanation_div.find('div', class_='wpProQuiz_unattempted')
            if unattempted_div:
                explanation_text = unattempted_div.get_text()
...
def extract_question_data(question_element):
    ...
    # Extract overall explanation - only from "Unattempted" section
    explanation_div = question_element.find('div', class_='wpProQuiz_response')
    if explanation_div:
        # Find specifically the "unattempted" div within the response
        unattempted_div = explanation_div.find('div', class_='wpProQuiz_unattempted')
        
        if unattempted_div:
            # Get the main explanation text from unattempted section only
            explanation_paragraphs = unattempted_div.find_all('p')
            explanation_parts = []
